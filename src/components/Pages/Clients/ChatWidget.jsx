...
import { MessageCircle, X, Send, Wifi, WifiOff, AlertCircle, Loader2, Sparkles, UserCheck, ChevronDown } from 'lucide-react';
...
const ChatWidget = () => {
  ...
  // 'ai' = AI đang phụ trách, 'human' = đã chuyển sang nhân viên
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      // Luôn gọi AI trừ khi đã chuyển sang chế độ nhân viên
      if (chatMode === 'ai') {
  ...
  // Gửi tin nhắn yêu cầu nhân viên hỗ trợ
  const handleRequestHuman = useCallback(async () => {
    if (!chatRoom || requestingHuman) return;
    ...
    try {
      await ChatService.sendMessage({
        ...
        senderId: customerId,
        senderType: SENDER_CUSTOMER,
        messageContent: '⚠️ Khách hàng yêu cầu được hỗ trợ bởi nhân viên.',
        messageType: 0,
      });
      setChatMode('human');
    } catch (err) {
      console.error('[ChatWidget] requestHuman error:', err);
    } finally {
      setRequestingHuman(false);
    }
  }, [chatRoom, customerId, requestingHuman]);
    ...
    <>
      {!isOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-xl flex items-center justify-center transition-transform hover:scale-105 active:scale-95"
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-blue-600 flex-shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-white font-semibold text-sm leading-tight truncate">
                  {chatMode === 'ai' ? 'Trợ lý AI PipeVolt' : 'Nhân viên hỗ trợ'}
                </p>
            ...
            <button onClick={handleClose} className="text-white/80 hover:text-white transition p-1 flex-shrink-0">
              <X className="w-5 h-5" />
            </button>
          </div>

          {/* Mode indicator bar — chỉ hiển thị khi đã chuyển sang nhân viên */}
          {chatMode === 'human' && (
            <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-50 border-b border-amber-100 flex-shrink-0">
              ...
              <span className="text-[11px] text-amber-700 font-medium">
                Nhân viên đang hỗ trợ bạn — AI đã tạm dừng
              </span>
            </div>
          )}
          ...
          {errorMsg && (
            <div className="bg-amber-50 border-b border-amber-200 px-3 py-2 flex items-center gap-2 flex-shrink-0">
              ...
              <span className="text-xs text-amber-700 flex-1 min-w-0">{errorMsg}</span>
              <button onClick={retryInit} className="text-xs text-amber-700 underline hover:text-amber-900 flex-shrink-0">
                Thử lại
              </button>
              <button onClick={() => setErrorMsg(null)} className="text-amber-400 hover:text-amber-600 flex-shrink-0">
          ...
          {!isLoggedIn ? (
            <div className="flex-1 flex flex-col items-center justify-center gap-3 p-6 text-gray-500 text-sm text-center">
              ...
              <p>Vui lòng đăng nhập tài khoản khách hàng để nhận hỗ trợ.</p>
              <Link
                to="/login"
                onClick={handleClose}
            ...
            <>
                ...
                {isInitializing ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full gap-2 text-gray-400">
                    ...
                    <p className="text-sm text-center px-2">
                      Xin chào! Tôi là trợ lý AI của PipeVolt.<br />
                      Hãy hỏi tôi về sản phẩm, giá cả, hoặc tư vấn lắp đặt!
                    </p>
                  </div>
                ) : (
                  <>
              ...
              {/* Input area */}
              <div className="border-t bg-white px-3 pt-2.5 pb-2 flex-shrink-0">
                <div className="flex items-end gap-2">
                  <textarea
                    className="flex-1 resize-none border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-50 leading-relaxed"
                    ...
                    rows={1}
                    maxLength={1000}
                    style={{ minHeight: '38px', maxHeight: '88px' }}
                    onInput={(e) => {
                ...
                {/* Footer: char count + request human button */}
                <div className="flex items-center justify-between mt-1">
                  <span className="text-[10px] text-gray-300">{inputText.length}/1000</span>
                  {chatMode === 'ai' ? (
                    <button
                      onClick={handleRequestHuman}
                      disabled={!chatRoom || requestingHuman}
                      className="flex items-center gap-1 text-[11px] text-gray-400 hover:text-blue-600 transition-colors disabled:opacity-40"
                      title="Yêu cầu nhân viên hỗ trợ trực tiếp"
                    >
                      {requestingHuman
